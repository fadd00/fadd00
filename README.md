...
usecaseDiagram
    ...
    package "Manajemen Thread (CRUD)" {
        usecase "Buat Thread Baru" as UC4
        usecase "Lihat Detail Thread" as UC5
        usecase "Cari Thread (Search)" as UC6
        usecase "Edit Caption Thread" as UC7
        usecase "Hapus Thread" as UC8
        usecase "Upload Gambar ke Storage" as UC_Img
    }

    package "Interaksi" {
        usecase "Kirim Komentar" as UC9
        usecase "Hapus Komentar Sendiri" as UC10
    }
    ...
    %% Relasi Include & Extend (Poin Plus Akademis)
    %% Include: Wajib kejadian
    UC4 ..> UC_Img : <<include>> 
    
    %% Extend: Opsi tambahan dari flow utama
    UC7 ..> UC5 : <<extend>>
    UC8 ..> UC5 : <<extend>>
    UC9 ..> UC5 : <<extend>>
